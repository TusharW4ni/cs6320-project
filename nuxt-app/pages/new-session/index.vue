...
const cuId = Number(currentUser.value.id);
...
async function getHTML(url: string) {
  ...
  try {
    ...
    // });
    // console.log({ res });
    const res = await $fetch(`/api/session/post`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ url, userId: cuId }),
    }).catch((e) => {
      useToastify(`Error: ${e}`);
        ...
        <form @submit.prevent="getHTML(url)">
          <div class="flex items-center space-x-4">
            ...
            <button
              v-if="!loading"
              type="submit"
              class="bg-emerald-600 text-white px-4 py-2 border-4 border-emerald-600 rounded-full bg-emerald-600 hover:bg-emerald-800 hover:border-emerald-800"
            >
